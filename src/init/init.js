...
async function getOneStructure(id) {
  ...
  const structure = await axios.get(
    `https://paysage.mesri.fr/json/Objets/${id}.json`,
  );
...
async function connect() {
  ...
  client.connect().catch((e) => {
    logger.info(
      `Connexion to mongo instance failed... Terminating... ${e.message}`,
    );
...
async function deleteCollection(db, collection) {
  ...
  if (allCollections.find((item) => item.name === collection)) {
    db.collection(collection).drop((err, delOK) => {
      if (err) throw err;
...
const getIds = (ids) => Object.keys(ids).map((ident) => ({
  id: ids[ident][0].Id,
  type: ident,
...
const getStructureV2 = (structureV1) => ({
  ...
  categories: getCategories(
    structureV1.Categorie ? structureV1.Categorie : null,
  ),
  websitePages: getWebsitePages(structureV1.Variables),
  socialMedia: getSocialMedia(structureV1.Internet || null),
  localisation: getLocalisation(
    typeof structureV1.Localisation[0] !== 'undefined'
      ? structureV1.Localisation[0]
      : null,
  ),
  createdBy: 'init',
  createdAt: getDate(),
