...
export async function exportFrEsrPaysageFonctionsGourvernance() {
  ...
  const json = data.map(({ structure = {}, person = {}, relationType = {}, ...relationship }) => {
    ...
    const row = {
      ...
      email: relationship.mandateEmail,
      telephone: relationship.mandatePhonenumber,
      date_debut: (!Number.isNaN(startDate.getTime())) ? startDate.toLocaleDateString('fr-FR', dateOptions) : undefined,
      date_fin: (!Number.isNaN(endDate.getTime())) ? endDate.toLocaleDateString('fr-FR', dateOptions) : undefined,
      date_fin_prevue: (!Number.isNaN(previsionalEndDate.getTime())) ? previsionalEndDate.toLocaleDateString('fr-FR', dateOptions) : undefined,
      date_maj: relationship.updatedAt || relationship.createdAt,
      annelis,
    };
    return row;
  });
