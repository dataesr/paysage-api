...
import { monitorStructuresIdentifiers } from './update-identifiers';
...
import {
  setIdentifierStatus,
  setStructureStatus,
} from './treatments/set-statuses';

const { taskName } = config.sirene;
const structuresIdentifiersTaskName = config.identifiers.taskName;
...
agenda.define(
  `${structuresIdentifiersTaskName}`,
  {
    shouldSaveResult: true,
    lockLifetime: 1000 * 60 * 60 * 2,
  },
  monitorStructuresIdentifiers,
);
...
agenda.on('complete', async (job) => {
  ...
  const keepFailureFields = job.attrs.failedAt && job.attrs.failedAt === job.attrs.lastFinishedAt;
