...
describe('API > email types > create', () => {
  ...
  it('ignore additionalProperties', async () => {
    await global.superapp
      ...
      .expect(201);
    const dbData = await global.db.collection('emailtypes').findOne({ id });
...
describe('API > email types > update', () => {
...
describe('API > email types > delete', () => {
  it('throws not found with wrong id', async () => {
    await global.superapp
  ...
  it('throws bad request if id still in use', async () => {
    await global.utils.db.collection('emails').insertOne({ emailTypeId: id });
    await global.superapp
      .delete(`/${resource}/${id}`)
      .set('Authorization', authorization)
      .expect(400);
    await global.utils.db.collection('emails').deleteMany({});
  });
});

describe('API > email types > list', () => {
  beforeAll(async () => {
    await global.utils.db.collection('emailtypes').deleteMany({});
    await global.superapp
      .post(`/${resource}`)
      .set('Authorization', authorization)
